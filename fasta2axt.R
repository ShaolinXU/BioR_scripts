...
output <- args[2] # 输出文件名称 output file name 

N_seq <- length(seq_file)

ref_seq <- seq_file[[which(attr(seq_file,"name")==args[3])]] #args[3] 指定用作参照的物种 taxa header as reference seq

ref_seq_seq <- ref_seq[1] # sequence of ref seq

for (i in 1:N_seq){
  if(attr(seq_file[[i]],"name")!=args[3]){
    comp_pair <- paste(args[3],attr(seq_file[[i]],"name"),sep = "-")
    write(comp_pair,file = args[2],append = T) # output file name in args[2]
